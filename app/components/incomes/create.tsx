...
export default function createIncome() {
  ...
  const createIncomeService = async () => {
    ...
    if (jwt && jwt.length>10){
        try {
        const response = await fetch("https://balance-tests.up.railway.app/incomes/create", {
            method: "POST",
            headers: {
        ...
        if (response.status === 400 || response.status === 500) {
            ...
            alert("Su ingreso " + data.concept +  " fue registrado satisfactoriamente.");  // Suponiendo que esto es lo que quieres en el caso de éxito
        }
        } catch (error) {
        alert("Error en la solicitud.");
        }
    ...
    <Screen>
      <View style={styles.formContainer}>
        <View style={styles.logoContainer}>
          ...
          <Text style={{marginTop: -30, fontWeight: "bold", color: "#226320"}}>INGRESOS</Text>
        </View>
        ...
        <Pressable onPress={createIncomeService} style={styles.inputButonStyle}>
          <Text style={styles.textInputLogin}>Crear Ingreso</Text>
        </Pressable>
      </View>
    </Screen>
  );
